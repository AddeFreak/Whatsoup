...
const CheckoutState = (props) => {
	...
	const removeFriendItem = async (product) => {
		...
		try {
			...
			let foundIndex = friendCheckout.findIndex(
			...
			await axios.delete(
				`https://ey-whatsoup.firebaseio.com/friend/${IDtoRemove}.json`
			);

			dispatch({
				type: REMOVE_FRIEND_ITEM,
				payload: { type: product.type, friend: friendCheckout },
			});
		} catch (err) {
			console.log(err);
			// dispatch({
